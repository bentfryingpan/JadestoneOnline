...
async function diagnose() {
	...
	if (matchErr) console.error('❌ Matches Table Error:', matchErr.message);
		...
		if (matchSamples?.length > 0) {
			...
			if (anyMatch?.length > 0) {
				console.log(
					'Found match with player_id:',
					anyMatch[0].player_id,
					'(Type:',
					typeof anyMatch[0].player_id,
					')'
				);
			}
		}
	...
	if (pErr) console.error('❌ Player Record Error:', pErr.message);
		...
		console.log(
			`✅ Player Record: Found ${playerRow.bungie_name}#${playerRow.bungie_code} (Games: ${playerRow.games_played})`
		);
