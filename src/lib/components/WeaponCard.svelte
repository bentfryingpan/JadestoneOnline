...
>
	...
	<header class="d2-weapon-tooltip__header">
		...
		<div class="d2-weapon-tooltip__meta">
			...
			<div class="d2-weapon-tooltip__type">
				<!-- damage dot -->
				<span
					class="d2-element-dot"
					style="background:{dmgColor};display:inline-block;width:7px;height:7px;border-radius:50%;margin-right:4px;vertical-align:middle;"
				></span>
				{dmgName} · {typeName}
			</div>
			{#if archetype}
			...
			<div style="display:flex;align-items:center;gap:0.5rem;margin-top:0.15rem;">
				<span
					style="font-family:var(--font-family-display);font-size:0.68rem;font-weight:600;letter-spacing:0.10em;text-transform:uppercase;color:{rColor}"
					>{rLabel}</span
				>
	...
	{#if screenshot && !compact}
		<div style="width:100%;height:80px;overflow:hidden;position:relative;">
			...
			/>
			<div
				style="position:absolute;inset:0;background:linear-gradient(to top, rgba(8,8,12,1) 0%, rgba(8,8,12,0) 60%);"
			></div>
		</div>
	{/if}
	...
	{#if !compact}
		...
		{#if perkCols.length}
			<div style="border-top:1px solid var(--d2-border-subtle);">
				{#each perkCols as col, ci (ci)}
					<div
						style="display:flex;padding:0.4rem 0.75rem;gap:0.4rem;border-bottom:1px solid var(--d2-border-subtle);align-items:flex-start;"
					>
		...
		{#if flavorText}
			<p
				style="padding:0.625rem 1rem;font-family:var(--font-family-sans);font-size:0.70rem;font-style:italic;color:var(--d2-text-muted);line-height:1.5;border-top:1px solid var(--d2-border-subtle);"
			>
				{flavorText}
			</p>
		{/if}
