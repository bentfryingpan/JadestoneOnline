...
<div style="display:flex;align-items:center;gap:0.6rem;">
    {#if icon}
    <div style="width:42px;height:42px;flex-shrink:0;border:1px solid var(--d2-border-default);background:rgba(255,255,255,0.06);overflow:hidden;">
        <img src={icon} alt={name} loading="lazy" style="width:100%;height:100%;object-fit:cover;" />
    </div>
    {/if}
    <div style="min-width:0;">
        <div class="d2-perk-name" style="font-size:0.80rem;color:var(--rarity-exotic, var(--d2-text-cream));">{name}</div>
...
<div style="display:flex;flex-direction:column;align-items:center;gap:0.3rem;width:44px;">
    ...
    >
        ...
        <img src={icon} alt={name} loading="lazy" style="width:100%;height:100%;object-fit:cover;" />
        {:else}
        <div style="width:100%;height:100%;background:rgba(255,255,255,0.08);border-radius:50%;"></div>
        {/if}
    </div>

    {#if showDesc && isSelected && (name || desc)}
    <!-- Tooltip-style name/desc shown below when selected -->
    <div style="width:120px;position:absolute;z-index:20;background:rgba(8,8,12,0.97);border:1px solid var(--d2-border-default);padding:0.4rem 0.5rem;pointer-events:none;display:none;">
