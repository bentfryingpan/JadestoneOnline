...
export const ALGO_CONFIG = {
	...
	pem_config: {
		...
		mote_efficiency_penalty_scale: 0.3,
...
export const MEDAL_ALIASES = {
	...
	notOnMyWatch: [
		'medalgambitsaviour',
		'medalsaviour',
		'medalgambitnotonmywatch',
		'medalspvecompmedaldenied',
		'medaldenied',
		'medalspvecompmedalinvasionshutdown'
	],
	...
	overkillmonger: [
		'medalgambitoverkillmonger',
		'medaloverkillmonger',
		'medalspvecompmedaloverkillmonger'
	],
...
const _MEDAL_REVERSE = (() => {
	...
	for (const [canon, aliases] of Object.entries(MEDAL_ALIASES)) {
		...
		for (const variant of [`medalgambit${fk}`, `medal${fk}`, `medalspvecompmedal${canon}`]) {
...
export function calculateEgoScore(stats) {
	...
	pve +=
		calcDR(chunks, 10000 * v.primevalDamage, dr) +
	...
	const floorBoost = buckets.reduce(
		(sum, b) => sum + Math.max(0, BUCKET_FLOOR - b) * FLOOR_GAIN,
		0
	);
	...
	if (eff > dbMote) {
		pem +=
			Math.floor((eff - dbMote) / conf.mote_efficiency_bonus_step) * conf.mote_efficiency_bonus_val;
	} else if (eff < dbMote) {
		const deficitFrac = (dbMote - eff) / 100.0;
		pem *=
			1.0 -
			Math.pow(deficitFrac, conf.mote_efficiency_penalty_power) *
				conf.mote_efficiency_penalty_scale;
	}
	...
	const simpleKd =
		((stats.mobKills ?? 0) + (stats.invasionKills ?? 0)) / Math.max(1, stats.deaths ?? 0);
...
export function detectRole(myScore, teamScores) {
	...
	const isCarry = myScore > teamTotal * 0.4 && myScore >= secondScore + 30;
	const isCarried = myScore < teamAvg * 0.5 && topScore >= myScore + 50;
