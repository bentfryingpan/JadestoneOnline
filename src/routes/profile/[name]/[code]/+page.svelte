<script>
    ...
    async function fetchCareer() {
        ...
        } catch (e) { console.error('Career fetch failed', e); }
    ...
    async function repairData() {
        if (!confirm('This will wipe your local intelligence cache and restart the Deep Scan. Proceed?')) return;
        deepScanning = true;
    ...
    const egoRating = $derived((() => {
        const wins    = (career?.source === 'supabase' ? (career.totalMatches * (career.winRate/100)) : (seasonalTotal?.wins ?? ltWon));
        const kills   = seasonalTotal?.kills ?? ltKills;
        const inv     = seasonalTotal?.invasionKills ?? ltInvKills;
        const motes   = (career?.source === 'supabase' ? (career.totalMatches * (career.avgMotes)) : (seasonalTotal?.motesDeposited ?? ltMotes));
    ...
    const weaponData = $derived((() => {
        ...
        if (career?.weapons) {
            career.weapons.forEach((w) => {
                const item = {
                    name: w.name, type: w.slot, kills: w.kills, precision: w.precRate + '%',
                    color: (w.winRate ?? 0) >= 60 ? 'text-emerald-400' : 'text-zinc-200',
                    icon: w.icon, hash: w.hash
                };
    ...
    const playerData = $derived({
        ...
        overview: {
            winRatio: (career?.source === 'supabase') ? career.winRate + '%' : (dWinRate != null ? fmtF(dWinRate, 1) + '%' : '—'),
            wins: (career?.source === 'supabase') ? Math.round(career.totalMatches * (career.winRate/100)) : dWon,
            kd: dKD != null ? fmtF(dKD, 2) : '—', 
            kills: dKills,
            motesAvg: (career?.source === 'supabase') ? career.avgMotes : (dEntered > 0 ? fmtF(dAvgMotes, 1) : '—'),
                                ...
                                <div class="flex items-center gap-6">
                                    ...
                                    <div>
                                        <p class="text-[12px] font-sans font-black text-emerald-500 uppercase tracking-[0.3em]">
                                            {deepScanStatus === 'syncing' ? 'Mirroring Manifest...' : deepScanStatus === 'discovering' ? 'Discovering History...' : 'Enriching Career Intelligence...'}
                                        </p>
                        ...
                        <div class="space-y-3 animate-in slide-in-from-bottom-2 duration-500 max-w-5xl mx-auto">
                            ...
                            {#each matchesList as m}
                                <a href="/match/{m.instanceId}" class="bg-[#0c0c0c] border border-zinc-800 p-3 group relative hover:border-emerald-500/50 transition-all flex items-center gap-6 font-sans block no-underline">
                                    ...
                                    {#if m.isEnriched}
                                        <div class="w-4 h-4 border border-emerald-500 rotate-45 flex items-center justify-center bg-emerald-500/10">
                            ...
                            {#if history.matches.length === 0 && !historyLoading}
                                <div class="col-span-full py-20 text-center text-zinc-700 uppercase tracking-[0.3em] text-[10px]">No matches loaded. Start a Deep Scan to begin.</div>
                            {/if}
                        </div>
                        ...
                        <div class="space-y-6 animate-in fade-in duration-700 max-w-6xl mx-auto">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    ...
                                    <div class="bg-[#111111] border border-zinc-800 p-5 relative shadow-[inset_0_0_30px_rgba(0,0,0,0.5)] group overflow-hidden transition-all duration-500 hover:border-zinc-600 font-sans stone-card">
                                         ...
                                         <div class="flex justify-between items-start mb-6">
                                                <div class="flex gap-4">
                                                    <div class="w-10 h-10 bg-zinc-900 border border-zinc-800 relative overflow-hidden">
                                                ...
                                                <a href="https://destinyitemmanager.com/en/inspect/{w.hash}" target="_blank" class="px-2 py-1 border border-emerald-500/20 text-[8px] text-emerald-500 font-black">DIM</a>
                                         </div>
                                ...
                                {#if !(weaponData[activeWeaponSlot]?.length)}
                                    <div class="col-span-full py-20 text-center text-zinc-700 uppercase tracking-[0.3em] text-[10px]">No recorded data. Perform a Deep Scan to fill this arsenal.</div>
                                {/if}
                            </div>
                        </div>
                                        ...
                                        <div class="bg-[#111111] border border-zinc-800 p-5 relative shadow-[inset_0_0_30px_rgba(0,0,0,0.5)] group hover:border-emerald-700/50 transition-all duration-500 stone-card">
                                            ...
                                            <div class="flex items-center relative z-10">
                                                ...
                                                <div class="ml-12 flex-1 font-sans">
                                                     <p class="text-sm font-bold text-zinc-100 tracking-wide uppercase">{teammate.name} <span class="text-zinc-600">#{teammate.code}</span></p>
                                    ...
                                    {#each (career?.rivals ?? []) as rival}
                                        <div class="bg-[#111111] border border-zinc-800 p-5 relative shadow-[inset_0_0_30px_rgba(0,0,0,0.5)] group hover:border-rose-900/50 transition-all duration-500 stone-card">
                                            ...
                                            <div class="flex items-center relative z-10">
                                                ...
                                                <div class="ml-12 flex-1 font-sans">
                                                     <p class="text-sm font-bold text-zinc-100 tracking-wide uppercase">{rival.name} <span class="text-zinc-600">#{rival.code}</span></p>
                                                     <div class="flex items-center gap-4 mt-1">
                                ...
                                {#each (pursuits?.records?.slice(0, 15) ?? []) as m}
                                    <div class="bg-[#111111] border border-zinc-800 p-5 relative group hover:border-zinc-600 transition-all font-sans stone-card">
                                         ...
                                         <div class="flex justify-between items-start mb-2 relative z-10">
                                                <div class="flex gap-4">
                                                    <img src={m.icon} alt={m.name} class="w-8 h-8 opacity-50 group-hover:opacity-100 transition-opacity" />
