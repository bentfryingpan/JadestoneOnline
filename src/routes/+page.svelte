...
>
	...
	<div
		style="position:absolute;top:32px;left:32px;width:20px;height:20px;border-top:1px solid var(--gambit-green);border-left:1px solid var(--gambit-green);opacity:0.4;pointer-events:none;"
	></div>
	<div
		style="position:absolute;top:32px;right:32px;width:20px;height:20px;border-top:1px solid var(--gambit-green);border-right:1px solid var(--gambit-green);opacity:0.4;pointer-events:none;"
	></div>
	<div
		style="position:absolute;bottom:32px;left:32px;width:20px;height:20px;border-bottom:1px solid rgba(255,255,255,0.12);border-left:1px solid rgba(255,255,255,0.12);pointer-events:none;"
	></div>
	<div
		style="position:absolute;bottom:32px;right:32px;width:20px;height:20px;border-bottom:1px solid rgba(255,255,255,0.12);border-right:1px solid rgba(255,255,255,0.12);pointer-events:none;"
	></div>
	...
	<div style="position:relative;text-align:center;margin-bottom:3rem;">
		<!-- Eyebrow -->
		<div
			style="display:flex;align-items:center;justify-content:center;gap:12px;margin-bottom:1.25rem;"
		>
			<div
				style="height:1px;width:48px;background:linear-gradient(to right,transparent,var(--gambit-green));opacity:0.6;"
			></div>
			<span
				style="
			...
			>
			<div
				style="height:1px;width:48px;background:linear-gradient(to left,transparent,var(--gambit-green));opacity:0.6;"
			></div>
		</div>
		...
		<div
			style="height:2px;width:80px;background:linear-gradient(90deg,transparent,var(--gambit-green),transparent);margin:0 auto 1.25rem;"
		></div>
	...
	<div style="position:relative;width:100%;max-width:440px;">
		...
		>
			<div style="display:flex;align-items:stretch;">
				<!-- Search icon -->
				<div
					style="padding:0 12px 0 16px;display:flex;align-items:center;color:var(--d2-text-muted);flex-shrink:0;"
				>
					<svg
				...
				{#if loading}
					<div style="margin-right:12px;display:flex;align-items:center;">
						<div
							style="width:12px;height:12px;border:1.5px solid rgba(255,255,255,0.15);border-top-color:var(--gambit-green);border-radius:50%;animation:spin 0.7s linear infinite;"
						></div>
					</div>
				{/if}
                ...
                "
					onmouseenter={(e) => {
						e.currentTarget.style.background = 'rgba(61,174,119,0.22)';
		...
		{#if error}
			<p
				style="font-size:0.72rem;color:#f87171;margin-top:8px;margin-left:4px;font-family:var(--font-family-display);letter-spacing:0.05em;"
			>
				{error}
			</p>
		{/if}
			...
			>
				{#each suggestions as s, i}
					...
					>
						...
						{:else}
							<div
								style="width:32px;height:32px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.10);flex-shrink:0;transform:rotate(45deg);"
							></div>
						{/if}
						<span
							style="font-family:var(--font-family-display);font-size:0.85rem;font-weight:500;letter-spacing:0.04em;color:var(--d2-text-primary);flex:1;text-align:left;"
						>
							{s.name}<span style="color:var(--d2-text-muted);">#{s.code}</span>
						</span>
						<svg
							style="width:10px;height:10px;color:var(--d2-text-muted);"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
