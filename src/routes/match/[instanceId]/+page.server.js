...
export async function load({ params, setHeaders }) {
    ...
    // ── Role detection within each team — exact desktop app thresholds ──────
    // Hard carry: top scorer AND >40% of team total AND ≥30 ahead of second
    // Carried:    score < 50% of team average AND top scorer ≥50 ahead of you
    function detectRoles(players) {
        const completed = players.filter(p => p.ego);
        if (completed.length < 2) return;
        const sorted    = [...completed].sort((a, b) => b.ego.finalScore - a.ego.finalScore);
        const teamTotal = sorted.reduce((s, p) => s + p.ego.finalScore, 0);
        const teamAvg   = teamTotal / sorted.length;
        const topScore  = sorted[0].ego.finalScore;
        const secondScore = sorted[1]?.ego.finalScore ?? 0;

        for (const p of completed) {
            const s = p.ego.finalScore;
            const isHardCarry = s === topScore
                && s > teamTotal * 0.40
                && s >= secondScore + 30;
            const isCarried = s < teamAvg * 0.50
                && topScore >= s + 50;
            p.is_hard_carry = isHardCarry;
            p.is_carried    = isCarried;
            // legacy role field for UI
            p.role = isHardCarry ? 'carry' : isCarried ? 'carried' : 'solid';
        }
    }
