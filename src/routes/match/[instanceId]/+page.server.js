...
export async function load({ params }) {
	...
	for (const e of pgcr.entries) {
		...
		const stats = {
			...
			medals: rawMedals,
			fireteamSize: pgcr.entries.filter(
				(oe) => oe.values?.fireteamId?.basic?.value === e.values?.fireteamId?.basic?.value
			).length
		};
	...
	return {
		...
		teamA: teams.Alpha,
		teamB: teams.Bravo,
		teamAWon: pgcr.entries.some(
			(e) =>
				teamMap[e.values?.team?.basic?.value] === 'Alpha' && e.values?.standing?.basic?.value === 0
		),
		teamBWon: pgcr.entries.some(
			(e) =>
				teamMap[e.values?.team?.basic?.value] === 'Bravo' && e.values?.standing?.basic?.value === 0
		)
	};
}
