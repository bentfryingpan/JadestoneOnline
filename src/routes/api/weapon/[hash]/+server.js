...
export async function GET({ params, url }) {
	...
	if (membershipId && membershipType) {
		try {
			...
			if (instanceId) {
				...
				if (plugData) {
					...
					for (const pHash of plugHashes) {
						...
						if (
							pDef &&
							(pDef.itemTypeDisplayName?.includes('Perk') ||
								pDef.itemTypeDisplayName?.includes('Frame') ||
								pDef.itemTypeDisplayName?.includes('Intrinsic') ||
								pDef.itemTypeDisplayName?.includes('Trait'))
						) {
							livePerks.push({
			...
			console.error('[live-inspect] Failed:', e.message);
		}
	}
	...
	if (item.sockets?.socketEntries) {
		...
		const allPerkDefs = await getDefs(
			'DestinyInventoryItemDefinition',
			Array.from(allPotentialPerkHashes)
		);
		...
		item.sockets.socketEntries.forEach((entry) => {
			...
			for (const pHash of hashes) {
				...
				if (
					pDef &&
					pDef.displayProperties?.name &&
					!pDef.displayProperties.name.includes('Empty')
				) {
					...
					if (
						...
						if (
							type.toLowerCase().includes('origin trait') ||
							name.toLowerCase().includes('origin trait')
						) {
	...
	const barStatHashes = [
		4043527740, 1240592695, 155624089, 943540823, 943549884, 4188034523, 4188031367, 4254817677,
		1345609583, 2715839340, 3871231018, 3871231066, 446212391, 2523465841, 1591432999, 3555963035,
		1931675084, 105267050, 2766642535, 1842278914, 3022301684
	];

	const valueStatHashes = [3893976251, 4284893193, 3614671103, 2961396640, 2837207746];
