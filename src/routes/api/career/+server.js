...
export async function GET({ url }) {
	...
	try {
		...
		for (const row of rows) {
			...
			for (const p of roster) {
				const isMe =
					String(p.id) === idStr ||
					(targetPrefix &&
						String(p.name).split('#')[0].toLowerCase() === targetPrefix &&
						String(p.code) === targetCodeStr);
				if (isMe) continue;
		...
		return json({
			...
			weapons: Object.entries(weaponsAgg)
				.map(([name, s]) => ({
					name,
					games: s.games,
					wins: s.wins,
			...
			allies: Object.values(playersAgg)
				...
				.sort((a, b) => b.games - a.games)
				.slice(0, 15),
			rivals: Object.values(playersAgg)
				...
				.sort((a, b) => b.games - a.games)
				.slice(0, 15),
			medals: Object.entries(medalsAgg)
