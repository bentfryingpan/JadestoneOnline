...
async function careerFromSupabase(membershipId, count) {
    ...
    for (const row of rows) {
        ...
        // Medal tally
        for (const [medal, cnt] of Object.entries(medals)) {
            if (cnt > 0) medalsAgg[medal] = (medalsAgg[medal] ?? 0) + cnt;
        }
        ...
        // Class stats from roster
        const myRosterEntry = (row.roster ?? []).find(r => r.is_target);
        const cls = myRosterEntry?.className;
        if (cls && classAgg[cls] !== undefined) {
            if (!classAgg[cls]) classAgg[cls] = { games: 0, wins: 0, scoreSum: 0 };
            classAgg[cls].games++;
            if (isWin) classAgg[cls].wins++;
            classAgg[cls].scoreSum += score;
        }
    }
