...
export async function GET({ url }) {
    ...
    const count        = Math.min(parseInt(url.searchParams.get('count') ?? '1000', 10), 10000);
    ...
    try {
        ...
        if (!result) {
            return json({ 
                ...
                medals: [], hourlyStats: [], classStats: [], matchesAnalyzed: 0,
                needsEnrichment: true
            });
        }
        return json(result);
    } catch (e) {
        return json({ error: e.message, source: 'error', totalMatches: 0, maps: [], weapons: [], allies: [], rivals: [], medals: [], hourlyStats: [], classStats: [] });
    }
}
