...
export async function GET({ setHeaders }) {
    ...
    try {
        ...
        for (const act of gambitActivities) {
            ...
            const pgcrImage = act.pgcrImage
                ? BUNGIE_ROOT + act.pgcrImage
                : null;
            const icon = act.displayProperties?.icon
                ? BUNGIE_ROOT + act.displayProperties.icon
                : null;
        ...
        const byNameFlat = Object.fromEntries(
            maps.map(m => [m.name, m.pgcrImage ?? m.icon ?? null])
        );
