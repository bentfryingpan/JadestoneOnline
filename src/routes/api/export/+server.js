...
const MEDAL_KEYS = [
    'notOnMyWatch','armyOfOne','locksmith','blockbuster','rapidPayback',
    'massacre','motesHaveBeen','halfBanked','firstToBlock','payback',
    'overkillmonger','killmonger','thrillmonger','fastFill','killAfterInvasion',
    'bigGameHunter','lastGuardianStanding','noEscape',
];
...
export async function GET({ url }) {
    ...
    const HEADER_ROW = csvRow([
        'Match #', 'Date', 'Map', 'Outcome', 'Stack Size',
        'Final RATING', 'Base PPS', 'PEM', 'Mote Eff %', 'K/D',
        'Mob Kills', 'Invasion Kills', 'Assists', 'Deaths',
        'Motes Deposited', 'Motes Denied', 'Motes Picked Up', 'Motes Lost',
        'Primeval Dmg', 'Hard Carry', 'Carried',
        ...MEDAL_KEYS,
    ]);
    ...
    try {
        const { data: rows } = await supabaseAdmin
            .from('player_matches')
            .select('pgcr_id,map_name,period,outcome,ego_score,ego_base,ego_pem,mote_eff,kd,fireteam_size,is_hard_carry,is_carried,stats')
    ...
    } catch { /* fall through to activity-history path */ }
    ...
    try {
        ...
        completed.forEach((act, i) => {
            ...
            const ego = calcEgo({ kills, deaths, assists, invasionKills, motesDeposited, motesDenied, motesPickedUp, motesLost, primevalDamage, fireteamSize: 1, medals: {} });
