...
export async function load({ params }) {
	...
	if (membershipIds.length) {
		const { data } = await supabaseAdmin
			...
			.select(
				'player_id, bungie_name, bungie_code, activities_entered, activities_won, kills, deaths, assists, invasions, invasion_kills, invasions_defeated, motes_deposited, motes_lost, kd_ratio, win_rate'
			)
	...
	const clanStats = withStats.length
		? {
				...
				avgWinRate: +(
					withStats.reduce((s, m) => s + m.gambit.winRate, 0) / withStats.length
				).toFixed(1),
