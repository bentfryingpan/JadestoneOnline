...
<div class="min-h-screen bg-[#080808] text-zinc-200">

    <!-- ── Clan hero ─────────────────────────────────────────────────────────── -->
    <div class="border-b border-zinc-800 bg-[#0a0a0a]">
        <div class="max-w-5xl mx-auto px-6 py-8">

            <!-- Back link -->
            <a href="/" class="inline-flex items-center gap-2 mb-6
                               text-[9px] font-mono uppercase tracking-[0.2em] text-zinc-600
            ...
            <div class="flex flex-col sm:flex-row items-start gap-6">
                ...
                <div class="shrink-0 relative w-20 h-20 flex items-center justify-center">
                    ...
                    <div class="absolute inset-0 flex flex-col items-center justify-center">
                        <span class="text-[8px] font-mono uppercase tracking-[0.2em] text-emerald-600 block">Clan</span>
                        <span class="text-[11px] font-mono font-bold text-emerald-400 leading-tight">[{data.callsign || '—'}]</span>
                    </div>
                    <!-- Corner accents -->
                ...
                <div class="flex-1 min-w-0">
                    <span class="text-[9px] font-mono uppercase tracking-[0.3em] text-emerald-500/60 block mb-1">
                        Clan Roster
                    </span>
                    ...
                    {/if}
                    <div class="flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-600 font-sans">
                    ...
                    {#if data.about}
                        <p class="text-sm text-zinc-400 font-sans mt-3 leading-relaxed max-w-xl">{data.about}</p>
                    {/if}
                </div>

                <!-- Aggregate clan stats -->
                {#if data.clanStats && trackedCount >= 3}
                    <div class="shrink-0 border border-zinc-800 bg-[#111111] relative overflow-hidden">
                        ...
                        <div class="grid grid-cols-2 divide-x divide-y divide-zinc-800">
                            {#each [
                                ...
                                <div class="px-5 py-3 text-center bg-[#0a0a0a]">
                                    <span class="text-xl font-mono font-bold block {s.hi ? 'text-emerald-400' : 'text-white'}">
                                        {s.val}
                                    </span>
                                    <span class="text-[8px] font-mono uppercase tracking-[0.2em] text-zinc-600 mt-0.5 block">
                                        {s.label}
                                    </span>
                                </div>
                            {/each}
    ...
    <div class="max-w-5xl mx-auto px-6 py-7">
        ...
        <div class="flex items-center justify-between mb-4 flex-wrap gap-3">
            <div class="flex items-center gap-3">
                <span class="text-[9px] font-mono uppercase tracking-[0.25em] text-zinc-500">Members</span>
                <span class="text-[9px] font-mono text-zinc-700 border border-zinc-800 px-2 py-0.5">
                    {filteredMembers.length}
                </span>
            </div>

            <!-- Filter toggle -->
            <div class="flex border border-zinc-800 overflow-hidden">
                {#each [['all','All'], ['tracked','Gambit data']] as [val, label]}
                    <button
                        onclick={() => filter = val}
                        class="px-3 py-1.5 text-[9px] font-mono uppercase tracking-[0.15em] transition-colors
                               {filter === val
                                   ? 'bg-zinc-900 text-zinc-200 border-r border-zinc-700'
        ...
        {#if trackedCount === 0}
            <div class="mb-5 flex items-start gap-3 border border-amber-500/20 bg-amber-950/10 px-4 py-3">
                <svg class="w-3.5 h-3.5 text-amber-400 shrink-0 mt-0.5"
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                          d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                </svg>
                <p class="text-xs text-amber-300/70 font-sans leading-relaxed">
        ...
        {:else if trackedCount < data.memberCount}
            <p class="text-xs text-zinc-600 font-sans mb-4">
        ...
        <div class="border border-zinc-800 bg-[#0a0a0a] relative overflow-hidden">
            ...
            <!-- Header row -->
            <div class="grid px-4 py-2.5 border-b border-zinc-800 bg-[#111111] select-none
                ...
                {#each [
                    ...
                    <button
                        ...
                        class="text-[8px] font-mono uppercase tracking-[0.2em] transition-colors {col.align}
                               {sortKey === col.key ? 'text-emerald-400' : 'text-zinc-600 hover:text-zinc-400'}">
            ...
            <div class="divide-y divide-zinc-800/50">
                {#each sortedMembers as member, i}
                    <a href="/profile/{encodeURIComponent(member.name)}/{member.code}"
                       class="grid px-4 py-3 items-center gap-2 hover:bg-zinc-900/40 transition-colors group
                              grid-cols-[2.5rem_1fr_4.5rem_4rem_4rem_4rem_5rem_5rem]">

                        <!-- Rank -->
                        <span class="text-[9px] font-mono text-zinc-700 text-center">
                        ...
                        <div class="flex items-center gap-3 min-w-0">
                            {#if member.icon}
                                <img src={member.icon} alt=""
                                     class="w-7 h-7 border border-zinc-800 object-cover shrink-0
                                            group-hover:border-emerald-500/30 transition-colors" />
                            {:else}
                                <div class="w-7 h-7 bg-zinc-900 border border-zinc-800 shrink-0
                                            flex items-center justify-center rotate-45">
                                    <span class="-rotate-45 text-[7px] font-mono text-zinc-600">?</span>
                                </div>
                            {/if}
                            <div class="min-w-0">
                                <p class="text-[11px] font-mono text-zinc-200 group-hover:text-emerald-400
                                          transition-colors truncate leading-tight">
                                    {member.name}
                                </p>
                                <p class="text-[8px] font-mono text-zinc-700 leading-tight">
                        ...
                        <div class="flex justify-center">
                            <span class="text-[8px] font-mono uppercase tracking-[0.1em] border px-1.5 py-0.5
                        ...
                        <!-- Win rate -->
                        <p class="text-[11px] font-mono text-center tabular-nums
                                  {member.gambit
                        ...
                        <!-- Games -->
                        <p class="text-[11px] font-mono text-center tabular-nums text-zinc-400">
                        ...
                        <!-- Last online -->
                        <p class="text-[9px] font-mono text-center text-zinc-700">
        ...
        <!-- Footer note -->
        <p class="text-xs text-zinc-700 font-sans text-center mt-4 leading-relaxed">
